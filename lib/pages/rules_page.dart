...
import 'package:movie_tool/pages/auto_rule_dialog.dart';
...
class _RulesPageState extends State<RulesPage> {
  ...
  void _showAutoGenerateDialog() async {
    final result = await showDialog<bool>(
      context: context,
      builder: (_) => const AutoRuleDialog(),
    );
  ...
  void _addFromClipboard() async {
    ...
    try {
      ...
      // If it looks like a URL, try auto-generate
      if (text.startsWith('http') || text.contains('.')) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('检测到URL，请使用"智能生成"功能')),
          );
        }
        return;
      }
          ...
          ? const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8),
                  Text('点击 ⚡ 智能生成', style: TextStyle(color: Colors.grey)),
                  SizedBox(height: 4),
                  Text('或点击 + 手动添加', style: TextStyle(color: Colors.grey)),
                ],
              ),
            )
          : ListView.builder(
